...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all vehicles in Default Group
    const defaultGroupVehicles = await db
      .select({
        id: vehicles.id,
        year: vehicles.year,
        make: vehicles.make,
      ...
      .where(eq(vehicleGroupAssignments.groupId, defaultGroup.id))
      .orderBy(desc(vehicles.updatedAt));

    return NextResponse.json({ vehicles: defaultGroupVehicles });
  } catch (error) {
