...
interface ReclaimVehiclesModalProps {
  isOpen: boolean;
  onClose: () => void;
  groupId: string;
  groupName: string;
...
export function ReclaimVehiclesModal({
  isOpen,
  onClose,
  groupId,
  groupName,
  onSuccess,
}: ReclaimVehiclesModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      loadVehiclesFromOtherGroups();
      setSelectedVehicleIds([]);
      setError("");
    }
  }, [isOpen]);

  const loadVehiclesFromOtherGroups = async () => {
    setLoading(true);
    try {
      // Fetch vehicles from non-default groups (excluding current group)
      const response = await fetch(
        `/api/vehicle-groups/non-default/vehicles?excludeGroupId=${groupId}`
      );
  ...
  const handleReclaimVehicles = async () => {
      ...
      await Promise.all(
        selectedVehicleIds.map((vehicleId) =>
          fetch(`/api/vehicle-groups/${groupId}/vehicles`, {
            ...
            if (!res.ok) {
              throw new Error("Failed to reclaim vehicle");
            }
            return res.json();
          })
        )
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-blue-700">
                <strong>Reclaim Mode:</strong> Move vehicles from customer
                groups back to the Default Group for reassignment.
              </p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            <p className="text-gray-500 text-sm">
              No vehicles in other groups to reclaim.
            </p>
          </div>
        ) : (
